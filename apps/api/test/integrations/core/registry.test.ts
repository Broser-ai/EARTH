...
import { PROVIDER_OPERATION_TYPES } from '../../../src/integrations/types.js';

function fakeAdapter(overrides: Partial<ProviderAdapter> = {}): ProviderAdapter {
  const base = new DisabledAdapter('ROBOFLOW');
  return {
    ...base,
...
describe('IntegrationRegistry controlled registration', () => {
  it('exposes explicit capabilities on the default disabled adapter', () => {
    ...
    expect(adapter.capabilities.allowedOperations).toEqual(['MATERIAL_IMAGE_INFERENCE']);
  ...
  it('refuses adapters that claim autonomous or booking operations', () => {
    ...
    expect(() =>
      registry.register(
        fakeAdapter({
          capabilities: {
            allowedOperations: ['MATERIAL_IMAGE_INFERENCE', 'BOOKING_SLOT'],
            externalDataTransfer: false,
            autonomousActions: false,
            maxTimeoutMs: 10_000,
            maxAttempts: 1,
    ...
    ).toThrow(/OPERATION_NOT_SUPPORTED|AUTONOMOUS_ACTION_FORBIDDEN/);

    expect(() =>
      registry.register(
        fakeAdapter({
          capabilities: {
            allowedOperations: ['MATERIAL_IMAGE_INFERENCE'],
            externalDataTransfer: false,
